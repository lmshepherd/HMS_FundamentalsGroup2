...
<body>
<header id="header"><h1>Health E-Records: Update Admin Information</h1></header>
<div id="container">
	...
	<br>
		<?php 
			...
			$this->table->add_row('Username ',
								$row1->username,
								form_open('update/change_username_admin').form_input('username',$this->input->post('username')),
								form_submit('username_change', 'Change Username').form_close());
			
			$this->table->add_row('Email ',
								$row1->email,
								form_open('update/change_email_admin').form_input('email',$this->input->post('email')),
								form_submit('username_change', 'Change Email').form_close()
					);
			
			$this->table->add_row('Home Phone ',
								$row1->homephone,
								form_open('update/change_homephone_admin').form_input('homephone',$this->input->post('homephone')),
								form_submit('homephone_change', 'Change Home Number').form_close()
					);
			
			$this->table->add_row('Work Phone ',
								$row1->workphone,
								form_open('update/change_workphone_admin').form_input('workphone',$this->input->post('workphone')),
